...
struct ExploreWriteService {
   static let shared = ExploreWriteService()
   
   func getExploreWrite(completion: @escaping (NetworkResult<Any>) -> Void) {
      ...
      let url = APIConstants.answerWriteURL
   ...
   private func judge(by statusCode: Int, _ data: Data) -> NetworkResult<Any> {
      ...
      guard let decodedData = try? decoder.decode(GenericResponse<ExploreWrite>.self, from : data) else { return .pathErr }
